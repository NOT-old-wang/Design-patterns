...
class TonyReception : public ReceiveParcel {
 public:
  TonyReception(std::string name, std::string phoneNum)
      : ReceiveParcel(name), phoneNum_(phoneNum) {}

  std::string getPhoneNum() { return phoneNum_; }
  void receive(std::string peaceContent) {
    std::cout << "货物主人:" << getName() << ", 手机号：" << getPhoneNum()
              << std::endl;
...
class WendyReception : public ReceiveParcel {
 public:
  WendyReception(std::string name, ReceiveParcel& receiveParcel)
      : ReceiveParcel(name), receiveParcel_(&receiveParcel) {}
 ...
 private:
  inline void preReceive() {
    std::cout << "我是" << receiveParcel_->getName()
              << "的朋友， 我来帮他拿快递！" << std::endl;
  }
